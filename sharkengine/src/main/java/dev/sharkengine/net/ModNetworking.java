...
public final class ModNetworking {
    ...
    public static void init() {
        ServerPlayConnectionEvents.DISCONNECT.register((handler, server) ->
                TutorialService.clearPlayerState(handler.player.getUUID()));
        ...
        PayloadTypeRegistry.playC2S().register(AnchorToggleC2SPayload.TYPE, AnchorToggleC2SPayload.CODEC);
        ...
        ServerPlayNetworking.registerGlobalReceiver(HelmInputC2SPayload.TYPE, (payload, ctx) -> {
            ...
            ctx.server().execute(() -> {
                ...
                ship.applyPilotInput(sp, payload.throttle(), payload.turn(), payload.forward());
            });
        });

        ServerPlayNetworking.registerGlobalReceiver(AnchorToggleC2SPayload.TYPE, (payload, ctx) -> {
            ServerPlayer sp = ctx.player();
            ctx.server().execute(() -> {
                if (!(sp.getVehicle() instanceof ShipEntity ship)) return;
                if (!ship.isPilot(sp)) return;
                ship.toggleAnchor(sp);
            });
        });
